...
import {
  ...
  AlertCircle, Activity, Lock, Server, Globe,
} from "lucide-react";
import { adminLogin, MOCK_ADMIN_CREDENTIALS } from "@/lib/auth";
      ...
      for (let a = 0; a < dots.length; a++) {
        for (let b = a + 1; b < dots.length; b++) {
          ...
          if (dist < 100) {
            ...
            ctx.strokeStyle = `rgba(239,68,68,${0.12 * (1 - dist / 100)})`;
            ctx.lineWidth = 0.6;
            ctx.stroke();
          }
        }
...
const widgets = [
  { label: "Active Leads",     value: "248",   change: "+12%",  Icon: Activity, pos: "top-[12%] left-[6%]",    delay: 0 },
  { label: "Monthly Revenue",  value: "₹4.2L", change: "+8%",   Icon: Globe,    pos: "top-[18%] right-[5%]",   delay: 0.25 },
  { label: "Moves This Month", value: "42",    change: "+24%",  Icon: Server,   pos: "bottom-[22%] left-[4%]", delay: 0.5 },
  { label: "Cities Active",    value: "100+",  change: "Live",  Icon: Globe,    pos: "bottom-[18%] right-[4%]", delay: 0.75 },
];
...
export default function AdminLoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const user = adminLogin(data.email, data.password, data.rememberMe);
    if (user) {
      // Full navigation so the proxy reads the fresh cookie on the next request
      window.location.href = "/admin";
    } else {
      setError(`Invalid credentials. Demo: ${MOCK_ADMIN_CREDENTIALS.email} / ${MOCK_ADMIN_CREDENTIALS.password}`);
    }
    setIsLoading(false);
  };

  return (
    /* Root: pitch-dark on all screen sizes */
    <div className="min-h-screen flex overflow-hidden bg-[#020617]">
      ...
      <div className="hidden lg:flex lg:w-[55%] relative overflow-hidden flex-col justify-center bg-[#020617]">
        ...
        {widgets.map(({ label, value, change, Icon, pos, delay }) => (
          <motion.div key={label}
            className={`absolute ${pos} bg-[#0F172A]/80 backdrop-blur-xl border border-white/8 rounded-xl px-4 py-3 min-w-[148px] shadow-2xl`}
            initial={{ opacity: 0, scale: 0.85, y: 16 }}
            animate={{ opacity: 1, scale: 1, y: [0, -7, 0] }}
            transition={{ delay: delay + 0.8, duration: 4.5, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }}
          >
            ...
            <div className={`text-[10px] font-semibold mt-1 ${change === "Live" ? "text-emerald-400" : "text-emerald-400"}`}>
              {change !== "Live" ? "↑ " : "● "}{change}
            </div>
          </motion.div>
        ))}
        ...
        <div className="relative z-10 px-14 max-w-[520px]">
          <motion.div initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            {/* Animated shield */}
            <motion.div
              className="w-[72px] h-[72px] bg-red-500/10 border border-red-500/25 rounded-[18px] flex items-center justify-center mb-10 shadow-lg"
              animate={{
                boxShadow: [
            ...
            <h1 className="text-[44px] lg:text-5xl font-bold text-white leading-[1.1] mb-4 tracking-tight">
              Admin Portal
            </h1>
            <p className="text-slate-400 text-[17px] leading-relaxed mb-10 max-w-sm">
              Secure access to the Sarkar Packers &amp; Movers management dashboard.
            </p>
            ...
            <div className="flex flex-wrap gap-2.5">
              {[
                ...
                { icon: Shield, label: "Session Protected" },
      ...
      {/* ── Right form panel ─────────────────────────── */}
      {/*
        Desktop: dark panel (#080f1e)
        Mobile:  full-screen pitch-dark with radial glow
      */}
      <div className="
        w-full lg:w-[45%]
        relative flex items-center justify-center
        bg-[#020617] lg:bg-[#080f1e]
        p-5 sm:p-8
      ...
      ">
        ...
        <div className="absolute inset-0 lg:hidden pointer-events-none">
          ...
          {/* Floating blur blobs */}
          <motion.div
            className="absolute top-1/4 left-1/4 w-40 h-40 bg-red-500/5 rounded-full blur-2xl"
            animate={{ scale: [1, 1.3, 1], opacity: [0.4, 0.7, 0.4] }}
            transition={{ duration: 5, repeat: Infinity }}
          />
          <motion.div
            className="absolute bottom-1/3 right-1/4 w-32 h-32 bg-indigo-500/5 rounded-full blur-2xl"
        ...
        >
          {/* Mobile header */}
          <motion.div className="flex flex-col items-center gap-3 mb-7 lg:hidden"
            initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
          >
            <motion.div
              className="w-14 h-14 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-center justify-center"
              animate={{ boxShadow: ["0 0 0px rgba(239,68,68,0)", "0 0 24px rgba(239,68,68,0.3)", "0 0 0px rgba(239,68,68,0)"] }}
              transition={{ duration: 3, repeat: Infinity }}
            >
            ...
            <div className="text-center">
              ...
              <p className="text-slate-500 text-xs mt-0.5">Authorized access only</p>
            </div>
          </motion.div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <input
                  {...register("email")}
                  type="email"
                  placeholder="admin@sarkarpackers.in"
                  autoComplete="email"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPw(!showPw)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-slate-400 transition-colors duration-200"
                  >
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href="/forgot-password" className="text-xs text-red-500/70 hover:text-red-400 transition-colors duration-200">
                  Forgot password?
                </Link>
              </div>
              ...
              >
                {isLoading ? (
                  ...
                  <><ShieldCheck className="w-4 h-4" />Access Dashboard<ArrowRight className="w-4 h-4" /></>
                )}
              </button>
            </form>
          </div>
          ...
          <div className="text-center mt-5 space-y-2">
            ...
            <Link href="/login" className="block text-xs text-slate-600 hover:text-slate-400 transition-colors duration-200">
              Customer login →
            </Link>
          </div>
        </motion.div>
...
// Fix missing Shield import used in security pills
